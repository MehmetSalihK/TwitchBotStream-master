...
$(function() {
    ...
    helpers.getEventMessage = function(event) {
        switch (event.type.toLowerCase()) {
            case 'subscriber':
                return (event.username + ' yeni abone oldum!');
            case 'prime subscriber':
                return (event.username + ' Twitch Prime ile yeni abone oldum!');
            case 'resubscriber':
                return (event.username + ' üst üste ' + event.months + ' aylığına yeniden abone oldu!');
            case 'follower':
                return (event.username + ' takip eti!');
            case 'bits':
                return (event.username + ' az önce ' + event.amount + ' bit alkışladım!');
            case 'host':
                return (event.username + ' az önce ' + event.viewers + ' izleyiciyle ağırlandı!');
            case 'auto-host':
                return (event.username + ' sadece ' + event.viewers + ' izleyiciyle otomatik olarak barındırılıyor!');
            case 'tip': // To be added soon.
                break;
            case 'raid':
                return (event.username + event.viewers + ' izleyici için baskın yapıldı!');
            case 'gifted subscription':
                return (event.username + event.recipient + ' ye abonelik hediye etti!');
        }
    };
